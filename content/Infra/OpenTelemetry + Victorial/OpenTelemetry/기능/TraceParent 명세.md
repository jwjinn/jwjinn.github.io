...
| **순서** | **구분**              | **값 (예시)**  | **의미 (해석)**                                                                                                                                                                                                                                                                       |
| ------ | ------------------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1**  | **버전**              | `00`        | **"이 형식은 W3C 표준 00버전입니다."**<br><br><br>나중에 표준이 바뀌면 이 숫자가 바뀔 수 있지만, 당분간은 무조건 `00`입니                                                                                                                                                                                                |
| **2**  | **Trace ID** ⭐️     | `4bf92f...` | **"전체 프로젝트 번호는 이겁니다."**<br><br>  <br>(32자리) 프론트엔드가 처음 만든 그 ID입니다. 끝까지 변하지 않습                                                                                                                                                                                                      |
| **3**  | **Parent ID** 👨‍👦 | `00f067...` | **"바로 직전에 나를 호출한 녀석의 ID입니다."**<br><br>(16자리)<br><br>• 프론트가 보낼 때: 프론트의 Span ID<br><br>• 백엔드A가 보낼 때: 백엔드A의 Spa                                                                                                                                                                      |
| **4**  | **Flags** 🚩        | `01`     **"녹화(Sampling) 여부입니다."**<br><br>• `01`: "이건 중요한 거니까 끝까지 기록해!" (Record)<br>> 뒤이어 통신하는 모든 서비스들이 `span data` 를 생성을 하고 opentelemetry를 통해서 데이터를 전송한다.<br>• `00`: "이건 그냥 넘겨, 기록하지 마." (Don't Record)<br>> 데이터를 opentelemetry로 전송하지 않는다.<br>> 다음 단계에도 알리기 위해서 `traceParent` 값만 전달을 한다. 데이터를  |
>[!tip]
