...
> 모델이라고 하는 것은 모형, 축소형의 의미로서 사람이 살아가면서 나타날 수 있는 다양한 현상에 대해서 일정한 표기법에 따라
> 표현해 놓은 모형이라고 할 수 있다.

> 사람이 살아가면서 접하는 다양한 현상을 모델링이라는 표기법에 따라 표기하는 것 자체를 의미합니다.<br>
> 즉, 모델을 만들어가는 일 자체를 모델링이라고 정의합니다.

내가 만약 비전공자의 입장에서 이 말을 처음 보았다면, 어떤 생각이 날까? 

계속 반문을 하고 있는 데, 이 말 하나하나를 한번에 이해하는 것은 솔직히 힘들 것 같아.
